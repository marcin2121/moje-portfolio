      ...
      <div className="w-full max-w-4xl mx-auto relative z-10 flex flex-col items-center">
        {status === 'success' ? (
          <motion.div 
            ...
            className="w-full max-w-2xl bg-green-500/10 border border-green-500/20 rounded-3xl p-8 md:p-12 text-center flex flex-col items-center gap-4"
          >
          ...
          <div className="w-full flex flex-col items-center relative">
            ...
            {step === 2 && (
              <button 
                onClick={() => setStep(1)} 
                className="absolute -top-16 left-0 md:-left-12 p-3 bg-zinc-900/80 backdrop-blur rounded-2xl border border-white/5 hover:border-white/20 transition-all text-zinc-400 hover:text-white active:scale-95 z-20 flex items-center gap-2 text-xs font-mono uppercase"
              >
            ...
            <AnimatePresence mode="wait">
              {step === 1 && (
                <motion.div key="step1" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="flex flex-col w-full">
                  <div className="text-center mb-10 md:mb-14">
                    <h2 className="text-3xl md:text-5xl lg:text-6xl font-black text-white tracking-tighter mb-4">
              ...
              {step === 2 && (
                <motion.div key="step2" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="flex flex-col w-full max-w-2xl mx-auto">
                  ...
                  <form onSubmit={handleSubmit} className="w-full flex flex-col gap-5 bg-zinc-900/50 backdrop-blur-xl border border-white/10 p-6 md:p-10 rounded-3xl shadow-2xl">
                    {status === 'error' && (
                      ...
                      {showNotes && (
                        <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="overflow-hidden">
                          <textarea 
                            ...
                            rows={3} 
                            disabled={status === 'loading'} 
                            className="w-full mt-2 p-4 md:p-6 bg-zinc-950/50 border border-white/10 rounded-xl md:rounded-2xl outline-none focus:border-white/30 text-zinc-200 text-xs md:text-sm font-mono resize-none transition-colors shadow-inner" 
                          />
                        </motion.div>
                      )}
                    </AnimatePresence>
                    ...
                    <button 
                      type="submit" 
                      disabled={status === 'loading'}
                      className="w-full mt-2 bg-orange-500 hover:bg-orange-400 text-zinc-200 font-black uppercase tracking-widest text-xs py-5 rounded-xl transition-all shadow-[0_0_30px_rgba(234,88,12,0.2)] flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                      {status === 'loading' ? (
                        <>
