...
export default function OfferPricing({ packages, companyName, slug = 'oferta' }: OfferPricingProps) {
  ...
  const [selectedPkgForAccept, setSelectedPkgForAccept] = useState<{ name: string; price: string; priceSuffix?: string } | null>(null);
  ...
  const handleOpenAcceptModal = (pkgName: string, price: string, priceSuffix?: string) => {
    ...
    setSelectedPkgForAccept({ name: pkgName, price, priceSuffix });
  };
  ...
  return (
    <>
      <section 
        ...
        className="py-20 md:py-28 px-4 relative overflow-hidden border-t border-slate-200/60"
      >
        <div className="max-w-7xl mx-auto relative z-10">
          <div className="text-center mb-16">
            <div className="text-xs font-mono tracking-[0.25em] text-orange-600 uppercase mb-3 font-semibold">
              ...
              return (
                <motion.div
                  ...
                  className={`relative flex flex-col bg-white rounded-3xl overflow-hidden transition-all duration-300 ${
                    pkg.highlighted 
                      ? 'border-2 border-orange-500 shadow-[0_20px_50px_rgba(249,115,22,0.12)] ring-4 ring-orange-500/10' 
                      : 'border border-slate-200 shadow-sm hover:shadow-md hover:border-slate-300'
                  }`}
                >
                  {pkg.badge && (
                    <div className="p-8 pb-0">
                      <span className="text-xs font-mono font-bold tracking-wider uppercase text-orange-600 bg-orange-50 px-3 py-1 rounded-md border border-orange-200 shadow-xs inline-block">
                        {pkg.badge}
                      </span>
                    </div>
                  )}

                  <div className={`p-8 pb-0 ${pkg.badge ? 'pt-4' : ''}`}>
                    <h3 className="text-2xl font-bold text-slate-900 mb-2 tracking-tight">{pkg.name}</h3>
                    <p className="text-slate-600 text-sm leading-relaxed font-light min-h-[3rem] mb-6">{fixOrphans(pkg.target)}</p>

                    <div className="mb-6">
                      {pkg.originalPrice && (
                        <div className="flex items-center gap-2 mb-1">
                          <span className="text-sm text-slate-400 line-through">{pkg.originalPrice} zł</span>
                          {pkg.discountBadge && (
                      ...
                      <div className="flex items-baseline gap-2">
                        <span className="text-4xl md:text-5xl font-black text-slate-900 tracking-tight">{pkg.price}</span>
                        <span className="text-slate-500 text-sm font-medium">{pkg.priceSuffix || 'zł (zw. z VAT)'}</span>
                      </div>
                    </div>
                  ...
                  <div className="px-8 pb-8 flex-1 flex flex-col bg-slate-50/50 mt-auto pt-6 border-t border-slate-100">
                    <ul className="space-y-4 mb-8 flex-grow text-sm leading-relaxed text-slate-700 font-light">
                      {pkg.features.map((feature, fIdx) => (
                        <li key={fIdx} className="flex items-start gap-3">
                          <Check className="shrink-0 mt-0.5 text-orange-500" size={18} />
                    ...
                    {isAccepted ? (
                      <div className="p-4 rounded-xl bg-emerald-50 border border-emerald-200 text-emerald-800 text-sm leading-relaxed">
                        <div className="flex items-center gap-2 font-bold mb-1 text-emerald-900">
                          <CheckCircle2 size={18} />
                          Pakiet zaakceptowany
                        </div>
                        <p className="text-emerald-700 text-xs font-light">
                          Dziękuję! Zgłoszenie zostało zarejestrowane. Przygotowuję umowę i odezwę się do Ciebie w ciągu kilku godzin.
                        </p>
                      </div>
                    ) : (
                      <button
                        onClick={() => handleOpenAcceptModal(pkg.name, pkg.price, pkg.priceSuffix)}
                        className={`w-full py-4 rounded-xl text-xs font-black uppercase tracking-widest transition-all duration-200 ${
                          pkg.highlighted
          ...
          <div className="mt-16 text-center">
            <div className="inline-flex flex-col sm:flex-row items-center justify-center gap-6 p-6 rounded-2xl bg-white border border-slate-200 shadow-sm max-w-2xl mx-auto">
              <div className="flex items-center gap-4 text-left">
                ...
                <div>
                  <p className="text-slate-900 font-bold text-sm">Masz pytania lub chcesz dopasować zakres prac?</p>
                  <p className="text-slate-500 text-xs font-light">Skontaktuj się ze mną bezpośrednio przed podjęciem decyzji.</p>
                </div>
              </div>
              <button
                onClick={() => {
                ...
                className="w-full sm:w-auto px-6 py-3 rounded-xl bg-slate-900 hover:bg-slate-800 text-white text-xs font-bold uppercase tracking-wider transition-all duration-200 shadow-sm shrink-0"
              >
                Zadaj pytanie
              </button>
            </div>
          ...
          <div className="flex flex-col sm:flex-row justify-center items-center gap-4 mt-12 relative z-20">
            <button 
              ...
              className="w-full sm:w-auto inline-flex justify-center items-center gap-2 px-8 py-3.5 rounded-xl bg-white border border-slate-200 text-slate-800 font-medium hover:border-orange-500 hover:text-orange-600 transition-all duration-200 shadow-sm"
            >
              {showGeneralOffer ? 'Ukryj ogólną ofertę' : 'Zobacz ogólną ofertę'}
            </button>
            <Link 
              href="/"
              className="w-full sm:w-auto inline-flex justify-center items-center gap-2 px-8 py-3.5 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-700 font-medium transition-all duration-200"
            >
              Strona główna
            </Link>
          </div>
        ...
        {selectedPkgForAccept && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
            <motion.div
              ...
              className="relative w-full max-w-lg p-8 rounded-3xl bg-white border border-slate-200 shadow-2xl"
            >
              <button
              ...
              <div className="text-xs font-mono tracking-widest text-emerald-600 font-bold uppercase mb-2">
              ...
              <p className="text-slate-600 text-sm mb-6">
                Wartość wdrożenia: <span className="text-slate-900 font-bold">{selectedPkgForAccept.price} {selectedPkgForAccept.priceSuffix || 'zł (zw. z VAT)'}</span> dla {companyName}
              </p>
              ...
              <form onSubmit={handleConfirmAccept} className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-4 py-3 rounded-xl bg-slate-50 border border-slate-200 text-slate-900 text-sm focus:outline-none focus:bg-white focus:border-orange-500 transition-colors"
                  />
                </div>
                ...
                <div>
                  ...
                  <textarea
                    ...
                    className="w-full px-4 py-3 rounded-xl bg-slate-50 border border-slate-200 text-slate-900 text-sm focus:outline-none focus:bg-white focus:border-orange-500 transition-colors resize-none"
                  />
                </div>
                ...
                <div className="pt-2">
                  <button
                    ...
                    className="w-full py-4 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-sm tracking-wide transition-all duration-200 disabled:opacity-50 flex items-center justify-center gap-2 shadow-md shadow-emerald-600/20"
                  >
                    {isAccepting ? 'Rejestrowanie zgłoszenia...' : 'Potwierdzam akceptację pakietu'}
                  </button>
                  <p className="text-[11px] text-slate-500 text-center mt-3 font-light">
        ...
        {showQuestionModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
            ...
            >
              ...
              <div className="text-xs font-mono tracking-widest text-orange-600 font-bold uppercase mb-2">
              ...
              {questionSent ? (
                <div className="p-6 rounded-2xl bg-emerald-50 border border-emerald-200 text-center">
                  ...
                  <h4 className="text-lg font-bold text-slate-900 mb-1">Pytanie zostało wysłane!</h4>
                  <p className="text-slate-600 text-sm font-light">
                    Dziękuję za wiadomość. Odpowiedź otrzymasz na podany adres e-mail najszybciej jak to możliwe.
                  </p>
                  <button
                    onClick={() => setShowQuestionModal(false)}
                    className="mt-6 px-6 py-2.5 rounded-xl bg-slate-900 hover:bg-slate-800 text-white text-xs font-semibold tracking-wider uppercase transition-colors"
                  >
                    Zamknij okno
                  </button>
                </div>
                ...
                <form onSubmit={handleSendQuestion} className="space-y-4">
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <input
                        ...
                        placeholder="Jan"
                        className="w-full px-3.5 py-2.5 rounded-xl bg-slate-50 border border-slate-200 text-slate-900 text-sm focus:outline-none focus:bg-white focus:border-orange-500 transition-colors"
                      />
                    </div>
                    <div>
                      <label className="block text-xs font-mono text-slate-500 mb-1 uppercase tracking-wider">
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      className="w-full px-3.5 py-2.5 rounded-xl bg-slate-50 border border-slate-200 text-slate-900 text-sm focus:outline-none focus:bg-white focus:border-orange-500 transition-colors resize-none"
                    />
                  </div>
                  ...
                  <div className="pt-2">
                    <button
                      ...
                      className="w-full py-3.5 rounded-xl bg-orange-500 hover:bg-orange-600 text-white font-bold text-sm tracking-wide transition-all duration-200 disabled:opacity-50 flex items-center justify-center gap-2 shadow-md shadow-orange-500/20"
                    >
                      <Send size={16} />
